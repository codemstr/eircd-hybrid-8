 ...
 * \version $Id: channel_mode.c 5758 2015-04-02 18:26:29Z michael $
 */
...
chm_operonly(struct Client *source_p, struct Channel *chptr, int parc, int *parn,
             char **parv, int *errors, int alev, int dir, char c, unsigned int d)
{
  if (alev < CHACCESS_HALFOP && !HasOFlag(source_p, OPER_FLAG_OPME))
  {
    if (!(*errors & SM_ERR_NOOPS))
      sendto_one_numeric(source_p, &me,
                         alev == CHACCESS_NOTONCHAN ? ERR_NOTONCHANNEL :
        ...
        char **parv, int *errors, int alev, int dir, char c, unsigned int d)
{
  ...
  if (alev < CHACCESS_CHANOP && !HasOFlag(source_p, OPER_FLAG_OPME))
  {
    if (!(*errors & SM_ERR_NOOPS))
      sendto_one_numeric(source_p, &me,
                         alev == CHACCESS_NOTONCHAN ? ERR_NOTONCHANNEL :
       ...
       char **parv, int *errors, int alev, int dir, char c, unsigned int d)
{
  ...
  if (alev < CHACCESS_CHANOP && !HasOFlag(source_p, OPER_FLAG_OPME))
  {
    if (!(*errors & SM_ERR_NOOPS) && !HasOFlag(source_p, OPER_FLAG_OPME))
      sendto_one_numeric(source_p, &me,
                         alev == CHACCESS_NOTONCHAN ? ERR_NOTONCHANNEL :
                         ERR_CHANOPRIVSNEEDED, chptr->name);
  ...
  /* no redundant mode changes */
  //sponji this will allow irc operators to op
